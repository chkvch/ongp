...
* normally commented-out.
      ...
      character (len=32) RHOCHAR, TCHAR
      IF(COMMAND_ARGUMENT_COUNT().NE.2)THEN
        WRITE(*,*)'ERROR, TWO COMMAND-LINE ARGUMENTS REQUIRED, STOPPING'
        STOP
      ENDIF
      CALL GET_COMMAND_ARGUMENT(1,RHOCHAR)
      CALL GET_COMMAND_ARGUMENT(2,TCHAR)
      READ(RHOCHAR,*) RHO ! char to real
      READ(TCHAR,*) T
C      write(*,'('' Input T (K): ''$)')
C      read*,T
C      write(*,112) ! write header
C   10 continue
C      write(*,'('' Input RHO [ 0 to stop]: ''$)')
C      read*,RHO
C      write(*,*) rho,t
      if (RHO.le.0.) stop
      call H2OFIT(RHO,T,PnkT,FNkT,UNkT,CV,CHIT,CHIR,PMbar,USPEC)
      SNk=UNkT-FNkT
      write(*,111) RHO,T,PMbar*100,PnkT,FNkT,UNkT,CV,CHIT,CHIR,SNk,USPEC
C      goto 10
      stop
  112 format('   rho[g/cc]    T [K]      P [GPa]   P/(n_i kT)',
     *  ' F/(N_i kT) U/(N_i kT) C_V/(N_i k)  chi_T     chi_r',
     *  '      S/(N_i k)')
  111 format(1P,4E12.4,12E11.3)
      end
